...
async def test_evaluation_report_has_human_and_machine_readable_forms() -> None:
    ...
    assert machine["metrics"]["total_queries"] == 19
    assert machine["category_metrics"]["exact-code"]["total_queries"] == 3
    assert "multi-document" in machine["category_metrics"]
    assert len(machine["queries"]) == 19
