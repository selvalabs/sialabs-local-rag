...
class EvaluationCategoryMetrics(BaseModel):
    total_queries: int
    query_success_rate: float
    document_recall_at_requested_k: float
    evidence_recall_at_requested_k: float
...
class EvaluationReport(BaseModel):
    ...
    category_metrics: dict[str, EvaluationCategoryMetrics]
...
async def run_evaluation(
    ...
    return EvaluationReport(
        ...
        category_metrics=_aggregate_category_metrics(results),
        queries=results,
    )
...
) -> dict[str, EvaluationCategoryMetrics]:
    grouped: dict[str, list[QueryEvaluationResult]] = {}
    for result in results:
        grouped.setdefault(result.category, []).append(result)
    return {
        category: EvaluationCategoryMetrics(
            total_queries=len(category_results),
            query_success_rate=_average_bool(result.success for result in category_results),
            document_recall_at_requested_k=_average(
                result.document_recall_at_k for result in category_results
            ),
            evidence_recall_at_requested_k=_average(
                result.evidence_recall_at_k for result in category_results
            ),
        )
...
def format_human_report(report: EvaluationReport) -> str:
    ...
    for category, category_metrics in report.category_metrics.items():
        lines.append(
            f"- {category}: queries={category_metrics.total_queries}, "
            f"success={category_metrics.query_success_rate:.4f}, "
            f"document_recall={category_metrics.document_recall_at_requested_k:.4f}, "
            f"evidence_recall={category_metrics.evidence_recall_at_requested_k:.4f}"
        )
    lines.extend([
        "",
        "Per-query results:",
