...
It does **not** define a second installer implementation. Artifact creation remains documented in [`INSTALLERS.md`](INSTALLERS.md), while the existing Windows local-app flow remains under [`../installer/windows/README.md`](../installer/windows/README.md) and issue #49.
...
- #52 — embedding compatibility and explicit reindex lifecycle;
- #54 — local data deletion semantics;
- #55 — deterministic retrieval evaluation harness;
- #53 — relevance gating measured against that evaluation.

The stronger knowledge-product layer is:

- #56 — hybrid dense + lexical retrieval;
- #57 — structure-aware ingestion and source locations;
- #58 — conversational retrieval separation.

Product expansion currently covered by the full regression suite includes:

- #59 — local folder collections and incremental indexing;
- #60 — richer Office formats and optional local OCR.

A future maintenance release does not need to reopen or reimplement these issues. Their behavior must remain protected by the current tests/evaluation on the release commit.
...
- [ ] Required reliability issues above are represented in the current `main` history.
...
- [ ] For the current Windows local-app flow, follow [`../installer/windows/README.md`](../installer/windows/README.md) / #49.
...
- replace the packaging architecture from #49;
